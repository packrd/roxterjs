...
try {
  ...
  if (err.code === "ENOENT") {
    fs.writeFileSync(fileRoute, `export default {}`);
    Log(`[ok]Created file router [green]*${FILE_PATH_ABSOLUTE}*`);
  } else Log("[error] ao verificar ou criar arquivo:", err);
}
...
export default async function ServerRoxter() {
  const Start = ({ setHeaders = [] } = {}) => {
    http
      .createServer(async (req, res) => {
        try {
          if (setHeaders && setHeaders[0]) {
            for (let { name, value } of setHeaders) res.setHeader(name, value);
          }
          ...
          const importRoutes = await import(
            pathToFileURL(FILE_PATH_ABSOLUTE).toString()
          );
          const ViewRoutes = importRoutes.default;

          const pathUrl = clearUrl(url)
            ?.split("/")
            ?.filter((u) => !!u)
            ?.join("/");
      ...
      .listen(PORT, HOST, () =>
        Log(
          `[ok] *Start RoxterJS* | Mode [green]*${MODE}* [white]| *Running* at [blue]*//${HOST}:${PORT}*`
        )
      );
  };
  return { Start };
